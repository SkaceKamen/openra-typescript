...
export class WAngle {
	...
	static zero: WAngle = new WAngle(0)
	...
	add(b: WAngle) {
		return new WAngle(this.angle + b.angle)
	}
	...
	sin(): number {
		return new WAngle(this.angle - 256).cos()
	}

	cos(): number {
		if (this.angle <= 256) {
			return WAngle.cosineTable[this.angle]
		}

		if (this.angle <= 512) {
			return -WAngle.cosineTable[512 - this.angle]
		}

		return -new WAngle(this.angle - 512).cos()
	}

	tan(): number {
		if (this.angle <= 256) {
			return WAngle.tanTable[this.angle]
		}

		if (this.angle <= 512) {
			return WAngle.tanTable[512 - this.angle] * -1
		}

		return new WAngle(this.angle - 512).tan()
	}
	...
	static arcTan(y: number, x: number, stride = 1): WAngle {
		if (y == 0) {
			return new WAngle(0)
		}
		...
		let bestAngle = 0
		for (let i = 0; i < 256; i = i + stride) {
			const val = Math.abs(1024 * ay - ax * WAngle.tanTable[i])
			if (val < bestVal) {
				bestVal = val
				bestAngle = i
			}
